...
tf.app.flags.DEFINE_integer("num_GPUs", os.environ.get('SM_NUM_GPUS'), "Number of GPUs in current host")
tf.app.flags.DEFINE_integer("num_CPUs", os.environ.get('SM_NUM_CPUS'), "Number of CPUs in current host")
...
def input_fn(filenames, channel='training', batch_size=32, num_epochs=1, perform_shuffle=False):
    ...
    if FLAGS.pipe_mode == 0:
        ...
        #dataset = dataset.prefetch(buffer_size=num_cpus) 
        #dataset = dataset.prefetch(buffer_size = num_cpus * 1024) 
...
def model_fn(features, labels, mode, params):
    ...
    #liangaws:为了每个训练实例能利用多GPU或者CPU来训练，用reuse=tf.AUTO_REUSE来wrapper model_fn函数。
...
def main(_):
    ...
    #liangaws:这里注释掉config设置，使用Sagemaker这里不需要设置device信息。
    """ 
    ...
    #config = tf.estimator.RunConfig().replace(session_config = tf.ConfigProto(log_device_placement=True))
    #DeepFM = tf.estimator.Estimator(model_fn=model_fn, model_dir=FLAGS.model_dir, params=model_params)
    
    #liangaws:测试实例多CPU来用replicate_model_fn对原model_fn做wrapper
    ...
    #下面需要在replicate_model_fn API中手动设置device，否则只是用一个CPU来做梯度更新，并没有起到多个CPU设备做tower的效果。replicate_model_fn会使用/CPU:0设备做梯度平均，因此下面设置的replicate_model_fn中的device把/CPU:0排除掉。
    #这里通过设置log_device_placement为True来查看是否tower会在多个CPU设备上做梯度更新。
    
    #manual_CPU_device_set = True
    manual_CPU_device_set = False
    
    num_cpus = int(os.environ['SM_NUM_CPUS'])
    
    config = tf.estimator.RunConfig().replace(session_config = tf.ConfigProto(allow_soft_placement=True, device_count={'CPU': num_cpus}, intra_op_parallelism_threads=num_cpus, inter_op_parallelism_threads=num_cpus))
    ...
    #不使用tower，直接使用CPU线程池来对op或者多个op做并行。在用TF estimator API PS分布式训练的时候，checkpoint保存路径必须是share的比如用S3的路径，否则会报错。
    ...
    elif FLAGS.num_CPUs > 1 and devices_list:  #这里排除掉CPU:0作为梯度平均，model计算的是number_cpus -1
        batch_size = FLAGS.batch_size * (FLAGS.num_CPUs - 1)
    else:
        batch_size = FLAGS.batch_size
        
    
    if FLAGS.task_type == 'train':
        train_spec = tf.estimator.TrainSpec(input_fn=lambda: input_fn(tr_files, channel='training', num_epochs=FLAGS.num_epochs, batch_size=batch_size))
        eval_spec = tf.estimator.EvalSpec(input_fn=lambda: input_fn(va_files, channel='evaluation', num_epochs=1, batch_size=batch_size), steps=None, start_delay_secs=1000, throttle_secs=1200)
        tf.estimator.train_and_evaluate(DeepFM, train_spec, eval_spec)
    elif FLAGS.task_type == 'eval':
        DeepFM.evaluate(input_fn=lambda: input_fn(va_files, num_epochs=1, batch_size=batch_size))
    ...
    if FLAGS.task_type == 'export' or FLAGS.task_type == 'train': 
        ...
        #liangaws: 在使用parameter server方式进行分布式训练的情况下，只需要master来保存模型。
