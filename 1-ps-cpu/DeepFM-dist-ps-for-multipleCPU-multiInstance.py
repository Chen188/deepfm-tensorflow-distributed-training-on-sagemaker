...
#4 Support export_model p[]f1or TensorFlow Serving
...
#liangaws:增加参数，一个是分布式训练需要的所有的主机名以及当前运行该程序的主机名。
...
def main(_):
    ...
    DeepFM = tf.estimator.Estimator(model_fn=model_fn, model_dir=FLAGS.model_dir, params=model_params, config=run_config)      

    batch_size = FLAGS.batch_size

    if FLAGS.task_type == 'train':
        train_spec = tf.estimator.TrainSpec(input_fn=lambda: input_fn(tr_files, channel=FLAGS.training_channel_name, num_epochs=FLAGS.num_epochs, batch_size=batch_size))
        eval_spec = tf.estimator.EvalSpec(input_fn=lambda: input_fn(va_files, channel=FLAGS.evaluation_channel_name, num_epochs=1, batch_size=batch_size), steps=None, start_delay_secs=1000, throttle_secs=1200)
        ...
        preds = DeepFM.predict(input_fn=lambda: input_fn(te_files, num_epochs=1, batch_size=batch_size), predict_keys="prob")
